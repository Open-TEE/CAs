...
static uint32_t omnishare_generate_root_key(uint8_t *key, uint32_t *key_size)
{
	OT_LOG(LOG_ERR, "GENERATING GENERATING WRITING ");

	memset(key, 'A', *key_size);
	return 0;
}

static uint32_t omnishare_init(uint8_t *root_key, uint32_t size)
{
	OT_LOG(LOG_ERR, "%s ::: %d", root_key, size);
	return 0;
}
			     ...
			     uint8_t *dest, uint32_t *dest_len)
{
	...
	OT_LOG(LOG_ERR, "DO CRYPTO : count %d, len %d, op_cmd %d", key_count, key_len, op_cmd);

	for (i = 0; i < key_count; i++) {
		for (j = 0; j < key_len; j++)
			OT_LOG(LOG_ERR, "%c", key_chain[(i * key_len) + j]);
	}
...
static int recurse_dir(char *parent_dir, uint8_t **key_chain, int *key_count, uint32_t *key_len)
{
	...
	OT_LOG(LOG_ERR, "RECURSE ENTRY : %s", parent_dir);
	...
	if (strcmp(parent_dir, root_dir) != 0) {
		*key_count += 1;
		...
		OT_LOG(LOG_ERR, "RETURNING %d", ret);
	} else {
		return 0;
	}
	...
	OT_LOG(LOG_ERR, "PATH JUST MADE : %s", abs_path);
	...
	if (*key_chain == NULL) {
		OT_LOG(LOG_ERR, "CREATING NEW KEYCHAIN");
		...
		*key_len = s_buf.st_size;
		OT_LOG(LOG_ERR, "CREATING NEW KEYCHAIN SIZE %d", *key_len);
	}
	...
	if (read(fd, key_chain[(*key_count - ret) * s_buf.st_size], s_buf.st_size) < 0)
		OT_LOG(LOG_ERR, "failed to read : %s", strerror(errno));

	close(fd);

out:
	free(tmp_path);
	OT_LOG(LOG_ERR, "RECURSE (%d): %s", *key_count, parent_dir);
	return ret - 1;
}
...
{
	OT_LOG(LOG_ERR, "GENERATE  : %s", parent_dir);
	...
	ret = omnishare_do_crypto(keychain, key_count, key_len, OM_OP_CREATE_DIRECTORY_KEY,
				  NULL, 0, key_buf, &key_len);
	if (ret){
		OT_LOG(LOG_ERR, "Failed to omnishare_do_crypto : %d", ret);
		return ret;
	}

	/* update the actual size of the key_buf */
	*key_buf_size = key_len;
	return 0;
}
